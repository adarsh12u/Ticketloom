...
import { getCurrentOrganizationContext, requireOrganizationMembership } from "@/lib/authz";
...
function cookieName(secure = shouldUseSecureAuthCookies()) {
    ...
    const token = await getToken({
      req: {
        headers: {
          ...
          authorization: options.tokenFromAuth ? `Bearer ${options.tokenFromAuth}` : undefined,
        },
      } as never,
 ...
 */
export async function authenticateSocketHandshake(handshake: {
...
export async function assertSocketPermission(auth: SocketAuthContext, permission: Permission) {
